...
**需求描述**:
在后端 (server) 实现一个简单的健康检查接口，用于监控服务运行状态。
...
**前端需求**:
- [ ] 创建 `Footer` 组件
- [ ] 显示内容: "© 2026 YJP Agent. All rights reserved."
- [ ] 样式: 居中显示，浅灰色字体，有一定的上下内边距
...
**前端需求**:
- [ ] 路径: `client/src/components/Button.tsx`
- [ ] 支持 `variant`: 'primary' | 'secondary' | 'outline'
- [ ] 支持 `size`: 'small' | 'medium' | 'large'
- [ ] 样式: 使用 CSS Modules 或内联样式，确保美观
...
**前端需求**:
- [ ] 路径: `client/src/components/Header.tsx`
- [ ] 左侧: 显示文字 Logo "YJP Agent"
- [ ] 右侧: 显示链接 "首页", "关于", "联系"
- [ ] 样式: 固定在顶部，背景白色，有下边框阴影
...
**需求描述**:
开发一个后端工具接口，用于对字符串进行简单的格式转换。

**后端需求**:
- [ ] 路径: `POST /api/utils/transform`
- [ ] 请求体: `{ "text": "hello", "type": "uppercase" }`
- [ ] 支持的 type:
    - `uppercase`: 转大写
    - `lowercase`: 转小写
    - `reverse`: 字符串反转
- [ ] 默认行为: 如果未提供 type，默认转换为大写。
